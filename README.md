[![Go Report Card](https://goreportcard.com/badge/github.com/LabMarket/roach)](https://goreportcard.com/report/github.com/LabMarket/roach)
[![license](https://img.shields.io/github/license/LabMarket/roach.svg)](https://github.com/LabMarket/roach/blob/master/LICENSE)
[![Release](https://img.shields.io/github/release/LabMarket/roach.svg)](https://github.com/LabMarket/roach/releases/latest)
...
You can install from source like so:
    ...
    git clone https://github.com/LabMarket/roach
    cd roach
    go install
...
`roach` supports all the basic arithmetic operation of `int` and `float` types.
...
`roach` contains two builtin containers: `array` and `hash`.
...
`roach` uses `fn` to define a function which will be assigned to a variable for
...
`roach` supports if-else statements.
...
`roach` supports the use of ternary expressions, which work as you
...
Roach supports the `switch` and `case` expressions, as the following example demonstrates:
...
`roach` supports a golang-style for-loop statement.
...
`roach` support two kinds of comments:
...
possible to define such methods in 100% roach!
...
[github-action-tester](https://github.com/LabMarket/github-action-tester) action.

Releases are automated in a similar fashion via [.github/build](.github/build),
and the [github-action-publish-binaries](https://github.com/LabMarket/github-action-publish-binaries) action.
...
```sh
go test -fuzztime=300s -parallel=1 -fuzz=FuzzRoach -v
```
