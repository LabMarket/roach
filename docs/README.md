...
* [Roach Programming Language](#roach-programming-language)
  * [Summary](#summary)
  * [Overview](#overview)
...
Roach is a toy language interpreter, written in Go. It has C-style syntax, and is largely inspired by Ruby, Python, Perl and c#
...
This is a sample program using roach language:
...
Roach support two kinds of single line comment and also block comment.
...
Roach supports 9 basic data types: `String`, `Int`, `UInt`, `Float`, `Bool`, `Array`, `Hash`, `Tuple` and `Nil`
...
In roach, there are mainly eleven types of constants(Literals).
...
Variables in Roach could start with the keyword `let`, or nothing with the
form `variable=value`.
...
In Roach, you can use enum to define constants.
...
### Meta-Operators
Roach has some meta-operators borrowed from perl6.
...
At the moment, Roach has six meta-operators：
* <p>~+</p>
* <p>~-</p>
* <p>~*</p>
* <p>~/</p>
...
```
### using statement
In roach, if you have some resources you want to release/free/close, e.g. close opened file, close network connection etc，
...
In roach, you are free to define some operators, but you cannot
...
In roach, integer is treated as an object, so you could call it's methods.
...
In roach, float is also treated as an object, so you could call it's methods.
Please see below examples:
...
In roach, decimal is Arbitrary-precision fixed-point decimal numbers.
...
In roach, you could use [] to initialize an empty array:
...
In roach, there are three types of `string`:
...
In roach, strings are utf8-encoded, you could use utf-8 encoded name as a variable name.
...
### Hash
In roach, the builtin hash will keep the order of keys when they are added to the hash, just like python's orderedDict.
...
In roach, Hash is also an object, so you could use them to operate on hash object:
...
In roach, `tuple` is just like array, but it could not be changed once it has been created.
...
### Extend basic type 
Roach also provides support for extending basic types.
...
### Optional type
Roach has support for Optional type like java8.
...
Roach has limited support for the oop concept, below is a list of features:
...
The roach parser could parse `public`, `private`, `protected`, but it has no effect in the evaluation phase.
That means roach do not support access modifiers at present.
...
In roach, all class is inherited from the root class `object`. 
...
Roach has support for class `indexer`(like c#). 
...
Roach also support class Category like objective-c（C# is called 'extension methods'）.
...
Roach also has very simple annotation support like java：
...
Roach also include some builtin annotations:
...
Maybe you are curious about why `nil` or `false` have message() function? Because in roach, `nil` and `false`
...
In roach, you could concatenate of different types. See below for examples:
...
Roach support list(array,string, range, tuple) comprehensions.
...
Roach also support hash comprehension.
...
Function in roach is a first-class object. This means the language supports passing functions as arguments to
...
Before ver5.0, Roach do not support multiple return values, But there are many ways to do it.
...
Starting from ver5.0, Roach support multiple return values using 'let'.
...
Roach has experimental support for working with `go` modules.

If you need to use go`s language package function, you first need to use `RegisterFunctions` or `RegisterVars` to
register `go` language functions or types into roach language.

Below is an example of `main.go`(extracted):

```swift
// Because in roach we already have built in module `fmt`, here we use `gfmt` for package name.
...
```
Now, in your roach file, you could use it like below:
...
For more detailed examples, please see `goObj.roach`.

## Standard module introduction

In roach, there are some standard modules provided for you. e.g. json, sql, sort, fmt, os, logger, time, flag, net, http, etc...
This is a brief introduction of some of the roach standard modules, don't expect it to be thorough.
...
In roach, the `linq` module support seven types of object:
...
Now, roach has a powerful `linq for file` support. it can be used to operate
...
In roach, regard to regular expression, you could use:
...
The formatter utility can format the roach language.
The highlighter utility can highlight the roach language to console or html.

You could also combine the two utilities:

```sh
./fmt xx.roach | ./highlight  //output to console(console highlight not support windows)
```
...
./mdoc -html -showsource -css 1 examples/doc.roach
...
./mdoc -html -showsource -css 1 -cssfile ./examples/github-markdown.css examples/doc.roach

//processing all the '.roach' files in examples directory, generate html.
./mdoc -html examples
```
