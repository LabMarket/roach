...
func TestFileObjects(t *testing.T) {
	tests := []struct {
		...
		{`let f = open("../parser/test_files/module.roach");str(f)`, "<file object: ../parser/test_files/module.roach>"},
		{`let f = open("../parser/test_files/module.roach");f.read()`, `import eval
import test
import sub_package`},
		{`let f = open("../parser/test_files/module.roach");f.readline()`, "import eval"},
		{`let f = open("../parser/test_files/module.roach");f.readline();f.readline()`, "import test"},
		{`let f = open("../parser/test_files/module.roach");f.readline();f.readline();f.readline()`, "import sub_package"},
	}
	d, _ := os.Getwd()
	fmt.Println(d)
...
`,
		...
		{`"abc" > "abc"`, "unsupported operator for infix expression: '>' and types STRING and STRING"},
		{`"abc" < "abc"`, "unsupported operator for infix expression: '<' and types STRING and STRING"},
		{`{"name":"Roach"}[fn(x) {x}];`, "key error: type FUNCTION is not hashable"},
	}
