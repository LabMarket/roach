...
func (p *Parser) ParseProgram() *ast.Program {
	...
	//if the roach file only have ';', then we should return earlier.
...
func (p *Parser) parseAssignExpression(name ast.Expression) ast.Expression {
	...
	case *ast.InfixExpression:
		/*
		   e.g. In a '*.roach' file, you only have below line:
		        c = 1 -
...
func (p *Parser) getImportedStatements(importpath string) (*ast.Program, map[string]*ast.FunctionLiteral, error) {
	...
	fn := filepath.Join(path, importpath+".roach")
	...
	if err != nil { //error occurred, maybe the file do not exists.
		...
		if len(importRoot) == 0 { //'MAGPIE_ROOT' environment variable is not set
			return nil, nil, fmt.Errorf("Syntax Error:%v- no file or directory: %s.roach, %s", p.curToken.Pos, importpath, path)
		} else {
			fn = filepath.Join(importRoot, importpath+".roach")
			e, err := ioutil.ReadFile(fn)
			if err != nil {
				return nil, nil, fmt.Errorf("Syntax Error:%v- no file or directory: %s.roach, %s", p.curToken.Pos, importpath, importRoot)
			}
			f = e
		}
	}
